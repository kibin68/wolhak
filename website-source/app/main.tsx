...
import {ArrowLeft,ArrowRight,ArrowUpRight,BookOpen,Maximize2,X} from 'lucide-react';
...
const show=(i:number)=>{setHeroView(false);setCurrent(i);setZoom(false);setOpen(true)};
return <><a className="skip" href="#album">앨범으로 바로 가기</a><div className="topline">1980학년도 · 제26회 졸업기념 <span>우리들의 이야기는, 여전히 이곳에.</span></div>
<header><a className="brand" href="#"><span className="seal">월학</span><span>월학국민학교<small>제26회 졸업생 추억 보관소</small></span></a><nav aria-label="주 메뉴"><a href="#school">우리 학교</a><a href="#album">졸업앨범</a><a href="#moments">그때 그 시절</a></nav><a className="header-link" href="#album">앨범 펼치기 <ArrowUpRight size={17}/></a></header>
<main><section className="hero" aria-labelledby="hero-title"><div className="hero-copy"><p className="eyebrow">기억 속 교문을 다시 열며</p><h1 id="hero-title">우리는 아직,<br/><em>그 교실에</em><br/>함께 있습니다.</h1><p className="intro">분필 냄새 나던 교실, 흙먼지 날리던 운동장.<br/>한 장의 사진을 펼치면<br/>그리운 얼굴들이 우리를 기다립니다.</p><a className="primary" href="#album"><BookOpen size={19}/> 우리의 졸업앨범 <ArrowRight size={19}/></a><div className="year"><span>1981</span><p>월학국민학교<br/>제26회 졸업생</p></div></div><div className="hero-photo"><span className="tape" aria-hidden="true"/><button className="photo-button" onClick={()=>{setHeroView(true);setZoom(false);setOpen(true)}} aria-label="우리 반 단체사진 크게 보기"><img src={photo('화질복원.png')} alt="교실 앞 계단에 함께 모인 월학국민학교 졸업생 단체사진" fetchPriority="high"/><span className="photo-enlarge"><Maximize2 size={17}/> 크게 보기</span></button><div className="caption"><span>참 반가운 얼굴들. 우리의 어린 날.</span><span>1981, 월학</span></div><div className="stamp" aria-hidden="true">졸업기념<br/><b>第 26 回</b><br/>1980 學年度</div></div></section>
<div className="motto" id="school"><span>그때의 교훈</span><p>부지런하고 <i>·</i> 참되고 <i>·</i> 정다운 어린이</p><span>오래도록, 마음에.</span></div>
